...
class CreateTlisteActionMarketingTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('tliste_action_marketing', function (Blueprint $table) {
            $table->string('CODE_UTILISATEUR', 20)->default('')->index('CODE_UTILISATEUR');
            $table->string('ID_TLISTE_ACTION_MARKETING', 32)->default('')->primary();
            $table->string('PID_ACTION_MARKETING', 32)->default('')->index('PID_ACTION_MARKETING');
    ...
    {
        Schema::dropIfExists('tliste_action_marketing');
    }
}
