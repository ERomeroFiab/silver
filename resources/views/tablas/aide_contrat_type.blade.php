...
@section('customcss')
    <style>
        #tabla_aide_contrat_type_filter {
            display: none;
        }
    </style>
@endsection
    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header">
                <h4 class="my-0">Tabla: <b>aide_contrat_type</b></h4>
                <p>(Total: {{ count( config('tablas')['aide_contrat_type'] ) }} columnas)</p>
            </div>
            <div class="card-body">
                <div class="row">
                    <div class="col-12" style="overflow-x: scroll;">
                        <table id="tabla_aide_contrat_type" class="table-hover" style="width:100%;">
                            <thead>
                                <tr>
                                    <th>1 TYPE</th>
...
@section('customjs')
    ...
    <script>
        let TABLA_aide_contrat_type;
        $(document).ready(function() {

            TABLA_aide_contrat_type = $('#tabla_aide_contrat_type').DataTable({
                serverSide: true,
                processing: true,
                ajax: {
                    url: "{{ route('get_tabla_aide_contrat_type') }}",
                ...
                buttons: [{
                    extend: 'excelHtml5',
                    title: "tabla aide_contrat_type - " + new Date().toLocaleString(),
                    className: "bg-info",
                    exportOptions: {
            ...
            function newExportAction(e, dt, button, config) {
                var self = this;
                var oldStart = dt.aide_contrat_type()[0]._iDisplayStart;
                dt.one('preXhr', function (e, s, data) {
                    // Just this once, load all data from the server...
                    data.start = 0;
                    data.length = 2147483647;
                    dt.one('preDraw', function (e, aide_contrat_type) {
                        ...
                        dt.one('preXhr', function (e, s, data) {
                            ...
                            aide_contrat_type._iDisplayStart = oldStart;
                            data.start = oldStart;
                        });
                ...
                // Requery the server with the new one-time export aide_contrat_type
                dt.ajax.reload();
            }
