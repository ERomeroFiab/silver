...
@section('customcss')
    <style>
        #tabla_aide_action_resultat_filter {
            display: none;
        }
    </style>
@endsection
    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header">
                <h4 class="my-0">Tabla: <b>aide_action_resultat</b></h4>
                <p>(Total: {{ count( config('tablas')['aide_action_resultat'] ) }} columnas)</p>
            </div>
            <div class="card-body">
                <div class="row">
                    <div class="col-12" style="overflow-x: scroll;">
                        <table id="tabla_aide_action_resultat" class="table-hover" style="width:100%;">
                            <thead>
                                <tr>
                                    <th>1 RESULTAT</th>
                                    <th>2 CATEGORIE</th>
...
@section('customjs')
    ...
    <script>
        let TABLA_aide_action_resultat;
        $(document).ready(function() {

            TABLA_aide_action_resultat = $('#tabla_aide_action_resultat').DataTable({
                serverSide: true,
                processing: true,
                ajax: {
                    url: "{{ route('get_tabla_aide_action_resultat') }}",
                ...
                buttons: [{
                    extend: 'excelHtml5',
                    title: "tabla aide_action_resultat - " + new Date().toLocaleString(),
                    className: "bg-info",
                    exportOptions: {
            ...
            function newExportAction(e, dt, button, config) {
                var self = this;
                var oldStart = dt.aide_action_resultat()[0]._iDisplayStart;
                dt.one('preXhr', function (e, s, data) {
                    // Just this once, load all data from the server...
                    data.start = 0;
                    data.length = 2147483647;
                    dt.one('preDraw', function (e, aide_action_resultat) {
                        ...
                        dt.one('preXhr', function (e, s, data) {
                            ...
                            aide_action_resultat._iDisplayStart = oldStart;
                            data.start = oldStart;
                        });
                ...
                // Requery the server with the new one-time export aide_action_resultat
                dt.ajax.reload();
            }
        ...
        //     TABLA_aide_action_resultat.draw();
        // });
