...
@section('customcss')
    <style>
        #tabla_aide_article_provider_filter {
            display: none;
        }
    </style>
@endsection
    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header">
                <h4 class="my-0">Tabla: <b>aide_article_provider</b></h4>
                <p>(Total: {{ count( config('tablas')['aide_article_provider'] ) }} columnas)</p>
            </div>
            <div class="card-body">
                <div class="row">
                    <div class="col-3 form-group">
                        <label>PROVIDER:</label>
                        <input id="input__PROVIDER" type="text" class="form-control">
                    </div>
                    <div class="col-3 form-group">
                ...
                <div class="row">
                    ...
                    <div class="col-12" style="overflow-x: scroll;">
                        <table id="tabla_aide_article_provider" class="table-hover" style="width:100%;">
                            <thead>
                                <tr>
                                    <th>1 PROVIDER</th>
...
@section('customjs')
    ...
    <script>
        let TABLA_AIDE_ARTICLE_PROVIDER;
        $(document).ready(function() {

            TABLA_AIDE_ARTICLE_PROVIDER = $('#tabla_aide_article_provider').DataTable({
                serverSide: true,
                processing: true,
                ajax: {
                    url: "{{ route('get_tabla_aide_article_provider') }}",
                    // error: function(jqXHR, ajaxOptions, thrownError) {
                    //     console.log("error: " + thrownError + "\n\n" + "status: " + jqXHR.statusText + "\n\n" + "response: "+jqXHR.responseText + "\n\n" + "options: "+ajaxOptions.responseText);
                    // },
                    data: function ( d ) {
                        d.SEARCH_BY_PROVIDER               = $('#input__PROVIDER').val();
                ...
                buttons: [{
                    extend: 'excelHtml5',
                    title: "tabla aide_article_provider - " + new Date().toLocaleString(),
                    className: "bg-info",
                    exportOptions: {
            ...
            function newExportAction(e, dt, button, config) {
                var self = this;
                var oldStart = dt.aide_article_provider()[0]._iDisplayStart;
                dt.one('preXhr', function (e, s, data) {
                    // Just this once, load all data from the server...
                    data.start = 0;
                    data.length = 2147483647;
                    dt.one('preDraw', function (e, aide_article_provider) {
                        ...
                        dt.one('preXhr', function (e, s, data) {
                            ...
                            aide_article_provider._iDisplayStart = oldStart;
                            data.start = oldStart;
                        });
                ...
                // Requery the server with the new one-time export aide_article_provider
                dt.ajax.reload();
            }

        });
        function buscar(){
            TABLA_AIDE_ARTICLE_PROVIDER.draw(); 
        }
        ...
        $( "#input__PROVIDER" ).change(function() { agregar_quitar_bg_success('input__PROVIDER'); });
